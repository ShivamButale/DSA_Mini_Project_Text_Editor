...
struct editorConfig {
  	int cx, cy;//controls the x and y coordinates i.e positions of the cursor
...
int editorReadKey() {//interpreting the various keypresses 
	int nread;
 	char c;
...
struct abuf {//new structure so that no need to call write again and again
	char *b;
  	int len;
};
...
void editorDrawRows(struct abuf *ab) {//printing the tildes to gain vi like interface
	int y;
...
void editorMoveCursor(int key) {//controlling the cursor position based on the keypresses
	switch (key) {
    		case ARROW_LEFT:
      			if (E.cx != 0) {
        			E.cx--;
...
void editorProcessKeypress() {//handling keypresses. It handles ctrl-q for quit, homekey
	int c = editorReadKey();
	switch (c) {
		case CTRL_KEY('q'):
	      		write(STDOUT_FILENO, "\x1b[2J", 4);
